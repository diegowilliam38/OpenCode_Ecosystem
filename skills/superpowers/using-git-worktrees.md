<!-- SAÍDA OBRIGATÓRIA: PORTUGUÊS BRASILEIRO FORMAL -->
<!-- Toda resposta ao usuário DEVE ser em português do Brasil formal. -->
<!-- Contexto em chinês para eficiência de tokens. Responda em PT-BR formal. -->
<!-- Modelo: big-pickle -->

---
name: using-git-worktrees
description: 1. Check current branch status
version: 1.0.0
author: ecosystem
category: superpowers
inspired_by: deer-flow 2.0 / opencode
compatibility: big-pickle
migrated_at: 2026-05-07 06:10:30
---

# Using Git Worktrees

# Process

1. Check current branch status
2. Create worktree: `git worktree add -b feature/name ../path`
3. Run project setup in new worktree
4. Verify clean test baseline
5. Work in isolation without affecting main workspace

## Workflow

### Step 1: Analisar
Entenda o problema.

### Step 2: Planejar
Defina passos e criterios.

### Step 3: Executar
Implemente solucao.

### Step 4: Verificar
Valide resultado.

## Best Practices

1. Validar antes de completar
2. PT-BR formal
3. Documentar decisoes
4. Verificacao automatica
5. Rastreabilidade

## Integration

| Component | Type | Connection |
|-----------|------|------------|
| nexus_integration | Script | Orquestracao |
| memory | MCP | Contexto |
| sequential-thinking | MCP | Raciocinio |
