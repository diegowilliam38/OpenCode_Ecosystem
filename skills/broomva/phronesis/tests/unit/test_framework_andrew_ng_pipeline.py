"""Andrew Ng ML Pipeline framework — Phase-1 B.14 instantiation."""
...
@pytest.fixture
def fw() -> Framework:
    return load_framework(FRAMEWORKS_ROOT / "ai-lifecycle" / "andrew-ng-pipeline.yaml")


class TestAndrewNgPipelineFramework:
    def test_loads_and_validates(self, fw: Framework):
        assert fw.id == "andrew-ng-pipeline"
        assert fw.category == "ai-lifecycle"
        assert "Andrew Ng" in fw.source_firm
    ...
    def test_relationships_complete(self, fw: Framework):
        assert "quantumblack-ml" in fw.relationships["similar_to"]
    ...
    def test_appears_in_load_all(self):
        ...
        assert "andrew-ng-pipeline" in load_all()
