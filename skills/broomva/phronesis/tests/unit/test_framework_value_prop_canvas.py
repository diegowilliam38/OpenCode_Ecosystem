"""Value Prop Canvas framework — Phase-1 B.9 instantiation."""
...
@pytest.fixture
def fw() -> Framework:
    return load_framework(FRAMEWORKS_ROOT / "ideation" / "value-prop-canvas.yaml")


class TestValuePropCanvasFramework:
    def test_loads_and_validates(self, fw: Framework):
        assert fw.id == "value-prop-canvas"
        assert fw.category == "ideation"
        assert "Strategyzer" in fw.source_firm
    ...
    def test_relationships_complete(self, fw: Framework):
        assert "jobs-to-be-done" in fw.relationships["similar_to"]
    ...
    def test_appears_in_load_all(self):
        ...
        assert "value-prop-canvas" in load_all()
