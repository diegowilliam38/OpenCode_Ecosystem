<!-- SAÍDA OBRIGATÓRIA: PORTUGUÊS BRASILEIRO FORMAL -->
<!-- Toda resposta ao usuário DEVE ser em português do Brasil formal. -->
<!-- Contexto em chinês para eficiência de tokens. Responda em PT-BR formal. -->
<!-- Modelo: big-pickle -->

---
name: email-best-practices
description: - **New app?** Plan which emails your app needs, then set up DNS authentication
version: 1.0.0
author: ecosystem
category: tooling
inspired_by: deer-flow 2.0 / opencode
compatibility: big-pickle
migrated_at: 2026-05-07 06:10:30
---

# Email Best Practices

# Start Here

- **New app?** Plan which emails your app needs, then set up DNS authentication
- **Spam issues?** Check authentication first — most common cause
- **Marketing emails?** Collect consent → Compliance → Marketing best practices
- **Production-ready?** Add retry + idempotency → webhooks → list management

More: https://github.com/resend/email-best-practices

## Workflow

### Step 1: Configurar
Instale dependencias.

### Step 2: Executar
Rode com parametros adequados.

### Step 3: Analisar
Verifique output.

### Step 4: Limpar
Remova temporarios.

## Best Practices

1. Verificar compatibilidade
2. Testar em ambiente isolado
3. Documentacao atualizada
4. Validar outputs
5. Limpar temporarios

## Integration

| Component | Type | Connection |
|-----------|------|------------|
| code-runner | Tool | Execucao |
| bash | Tool | Sistema |
| filesystem | MCP | Arquivos |
